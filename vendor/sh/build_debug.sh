...
cd /data/www/social_backend_pay
if [ ! -d  ./bin ]
then
    mkdir ./bin
else
...
NAME="social-pay-http"
go build -tags "debug" -o ./bin/$NAME-server ./server/http_server.go
NAME="social-pay-rpc"
go build -tags "debug" -o ./bin/$NAME-server ./server/rpc_server.go
NAME="social-pay-recallback"
go build -tags "debug" -o ./bin/$NAME-server ./server/recallback_server.go
